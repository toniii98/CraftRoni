...
import { ChevronLeft, Lock, ShoppingCart, UserRound } from "lucide-react";
...
interface CheckoutPageClientProps {
  authenticatedUser: AuthenticatedUserSummary | null;
}

function createInitialForm(authenticatedUser: AuthenticatedUserSummary | null): FormData {
  const address = authenticatedUser?.defaultAddress;
...
export function CheckoutPageClient({ authenticatedUser }: CheckoutPageClientProps) {
  const router = useRouter();
  const { cart, clearCart } = useCart();
  const [isLoading, setIsLoading] = useState(false);
  const [formData, setFormData] = useState<FormData>(() => createInitialForm(authenticatedUser));
  const [errors, setErrors] = useState<FormErrors>({});

  const isAuthenticated = Boolean(authenticatedUser);
  const accountRedirect = useMemo(() => encodeURIComponent("/zamowienie"), []);
  ...
  const resetToAccountDefaults = () => {
    setFormData(createInitialForm(authenticatedUser));
    setErrors({});
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8 flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Zamówienie</h1>
          <p className="mt-2 text-muted">
            {isAuthenticated
              ? "Kupujesz jako zalogowany klient. Zamówienie zostanie przypisane do Twojego konta."
              : "Możesz złożyć zamówienie jako gość albo zalogować się, by przyspieszyć kolejne zakupy."}
          </p>
        </div>
        {isAuthenticated ? (
          <div className="rounded-xl border border-primary/20 bg-primary/5 p-4 text-sm text-foreground">
            ...
            <p className="mt-2">{authenticatedUser?.email}</p>
            <div className="mt-3 flex flex-wrap gap-2">
              <Button type="button" variant="outline" size="sm" onClick={resetToAccountDefaults}>
                Przywróć dane z konta
              </Button>
              <Link href="/konto/adresy">
                <Button type="button" variant="ghost" size="sm">Zarządzaj adresami</Button>
              </Link>
            </div>
          </div>
        ) : (
          <div className="rounded-xl border border-border bg-surface p-4 text-sm text-foreground">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <Link href={`/konto/logowanie?redirect=${accountRedirect}`}>
                <Button type="button" size="sm">Zaloguj się</Button>
              </Link>
              <Link href={`/konto/rejestracja?redirect=${accountRedirect}`}>
                <Button type="button" size="sm" variant="outline">Załóż konto</Button>
              </Link>
            </div>
          </div>
        )}
